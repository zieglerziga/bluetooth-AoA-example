...
/**
 * The following addresses are derived from the PK engine data sheet,
 * the micro code memory overview.
 */

/**
 * ucode content: memory size (4096) - redundancy value size (2)
 */
#define BA414EP_UCODE_MAX_LENGTH 4094

/**
 * Memory address where the microcode size is stored (16 bits).
 */
#define BA414EP_UCODE_SIZE_ADDR ((ADDR_BA414EP_UCODE) + 2 * 128)

/**
 * Memory address where the available microcode feature bitmap is stored (64 bits).
 */
#define BA414EP_UCODE_AVAILABLE_FEATURES_ADDR ((ADDR_BA414EP_UCODE) + 2 * 128 + 4)
#define BA414EP_MAX_FEAT_ID 63

#define BA414EP_UCODE_DIGEST_SIZE 4
 ...
 * @param operandsize the operands size in bytes (up to 2^10 bytes)
 ...
 * @return CRYPTOLIB_UNSUPPORTED_ERR if the operand size encoding exceeds the allocated bits in the
 *         command register; CRYPTOLIB_SUCCESS otherwise.
 */
uint32_t ba414ep_set_command(uint32_t op, uint32_t operandsize, uint32_t swap, uint32_t curve_flags) CHECK_RESULT;
...
#define BA414EP_CMD_RANDMOD_LSB     (19)
...
#define BA414EP_CMD_RANDMOD_MASK    (0x00080000)
#define BA414EP_CMD_OPSIZE_MASK     (0x0003FF00)
...
#define BA414EP_CMD_RANDMOD(v) (((v) << BA414EP_CMD_RANDMOD_LSB)&BA414EP_CMD_RANDMOD_MASK)
...
#define BA414EP_OPTYPE_MILLER_RABIN            (0x42)
   ...
   #define BA414EP_IS_OP_WITH_EXPO_CM(op)      ((op == BA414EP_OPTYPE_RSA_CRT_DEC)          || \
   ...
   #define BA414EP_IS_OP_WITH_PTMUL_CM(op)      ((op == BA414EP_OPTYPE_ECC_POINT_MULT)          || \
                                                (op == BA414EP_OPTYPE_ECDSA_SIGN_GEN)          || \
                                                (op == BA414EP_OPTYPE_ECKCDSA_PK_GEN)          || \
                                                (op == BA414EP_OPTYPE_SM2_SIGN_GEN))
#endif
...
extern const volatile uint32_t * const ADDR_BA414EP_HW_CFG;
